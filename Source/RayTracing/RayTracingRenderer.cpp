...
RayTracingRenderer::RayTracingRenderer(RayTracingCamera* InCamera, RayTracingScene* InScene): Camera(InCamera), Scene(InScene), RayPerPixel(RAY_PER_PIXEL) {
}

void RayTracingRenderer::Render() {
	LOG_INFO("Scene is Rendering...");
	const float SampleScale = 1.0f / (float)RayPerPixel;
	...
	ParallelFor(0ull, NumPixels, [this, SampleScale, RenderSize](size_t Pixel) {
		...
		for (uint32 Sample = 0; Sample < RayPerPixel; ++Sample) {
			const Math::FRay ray = Camera->GetRandomRay(i, j);
			Color += ComputeRayResult(ray, RAY_RECURSIVE_DEPTH);
		}
		Color *= SampleScale;
