...
bool DielectricMaterial::Scatter(const Math::FRayWithTime& InRay, const Math::FRayHit& RayHit, FScatterRecord& OutRec) const {
	...
	const float ReflectanceValue = Reflectance(CosTheta, ReflectionIndex);
	if (ReflectanceValue > Math::Random01()) {
		IsReflection = true;
	}
	if (!IsReflection) {
		float SinTheta = Math::Sqrt(1.0f - CosTheta * CosTheta);
		float SininThetaRefracted = ReflectionIndex * SinTheta;
...
bool IsotropicMaterial::Scatter(const Math::FRay& Ray, const Math::FRayHit& RayHit, Math::FVector4& OutColor, Math::FRay& OutRay, float& OutPDF) const {
	OutRay=Math::FRay(RayHit.Position, Math::RandomUnitVector());
	OutColor = Texture->SampleVector4(RayHit.Texcoord, RayHit.Position);
	return true;
}
