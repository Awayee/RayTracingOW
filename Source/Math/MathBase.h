...
#define ATRI_FUNC(func, ...) { return func( ##__VA_ARGS__); }
	...
	//data pack
	inline unsigned char PackSNorm(float x) { return (unsigned char)((x * 0.5f + 0.5f) * 255.0f); }
	inline float UnpackSNorm(unsigned char x) { return (float)x / 255.0f * 2.0f - 1.0f; }
	inline unsigned char PackUNorm(float x) { return (unsigned char)((x) * 255.0f); }
	inline float UnpackUNorm(unsigned char x) { return (float)x / 255.0f; }
