...
namespace Math {
	template<typename T> struct TransformWithoutScale {
		Vector3<T> Position{ 0,0,0 };
		Vector3<T> RotationEuler{0,0,0}; // in radian

		void Accumulate(const TransformWithoutScale& rhs) {
			RotationEuler = rhs.RotationEuler + RotationEuler;
			Position += rhs.Position;
		}

		bool IsIdentity() const {
			return Position == Vector3<T>::ZERO && RotationEuler == Quaternion<T>::IDENTITY;
		}

		Math::Matrix4x4<T> ToMatrix() const {
			Math::Matrix4x4<T> matrix;
			matrix.MakeTransform(Position, FVector3::ONE, RotationEuler);
			return matrix;
		}
	};
